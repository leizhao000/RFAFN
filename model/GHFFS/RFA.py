...
class LFIFN(nn.Module):
    def __init__(self, in_channels=3 ,out_channels=64, scale=4):
        ...
        self.c = B.conv_layer(out_channels *4 , out_channels, kernel_size=1)  # 特征聚合
        self.c1=B.conv_layer(out_channels ,out_channels, kernel_size=3)  # 特征聚合
    ...
    def forward(self,input):
        ...
        B3 = self.HFFM3(B2)
        B4 = self.HFFM4(B3)
        out_B = self.act(self.c(torch.cat([B1,B2,B3,B4], dim=1)) ) # 特征聚合

        out_lr =out_fea+self.c1(out_B)
