...
class SCAB(nn.Module):
    def __init__(self,in_channels):
        ...
        self.cam=B.CCALayer(in_channels)
...
class LFIFN(nn.Module):
    def __init__(self, in_channels=3 ,out_channels=64, scale=4):
        ...
        self.upsampler = B.pixelshuffle_block(out_channels, in_channels, upscale_factor=scale)

    def forward(self,input):
        out_fea=self.fea_conv(input)
        out_pixel=F.interpolate(input, scale_factor=self.scale, mode='bilinear',align_corners=False)
        B1 = self.HFFM1(out_fea)
        B2 = self.HFFM2(B1)
        B3 = self.HFFM3(B2)
        B4 = self.HFFM4(B3)
        out_lr =out_fea+self.c(B4)
        output =self.upsampler(out_lr)+out_pixel
