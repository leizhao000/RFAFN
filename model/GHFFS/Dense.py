...
class LFIFN(nn.Module):
    def __init__(self, in_channels=3 ,out_channels=64, scale=4):
        ...
        self.c2 = B.conv_layer(out_channels * 3, out_channels, kernel_size=1)  # 特征聚合
        self.c3 = B.conv_layer(out_channels * 4, out_channels, kernel_size=1)  # 特征聚合
        self.c4= B.conv_layer(out_channels * 5, out_channels, kernel_size=1)  # 特征聚合
    ...
    def forward(self,input):
        ...
        B2 = self.HFFM2(c1)
        c2 = self.act(self.c2(torch.cat([out_fea,B1, B2], dim=1)))

        B3 = self.HFFM3(c2)
        c3 = self.act(self.c3(torch.cat([out_fea,B1,B2, B3], dim=1)))

        B4 = self.HFFM4(c3)
        out_B = self.act(self.c4(torch.cat([out_fea,B1,B2, B3,B4], dim=1)) ) # 特征聚合
